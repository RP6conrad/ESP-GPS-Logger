...
void SetFont_Speed_I2(){
  if(displayHeight<130){display.setFont(&impact50pt7b);Font=100;           NumPix=Font*2*3/4; TotPix=Font*2;} //moet 50 worden
  else                 {display.setFont(&impact60pt7b);Font=120;           NumPix=Font*2*3/4; TotPix=Font*2;}
}
...
void Off_screen(int choice){//choice 0 = old screen, otherwise Simon screens
  ...
  if(choice==0){
    SetESPGPSLogo();
    CURSORC(1,Font); display.print("ESP-GPS saving");
   if(Shut_down_Save_session==true){
      CURSORC(2,Font); display.print("Saving session");
      CURSORC(3,Font); display.print("Time: "); display.print(session_time,0); display.print(" s");
      CURSORC(4,Font); display.print("AVG: ");display.print(RTC_avg_10s,2);
      CURSORC(5,Font); display.print("Dist: ");display.print(Ublox.total_distance/1000,0);
    }
    else{
    CURSORC(2, Font); display.print("Going back to sleep");
    }
  }
  else{
...
//  display.setCursor(displayWidth-8,(InfoBarRow-ROW_1M));
  ...
  if (RTC_bat_choice==1){
    if(displayHeight<130) {
      if (bat_perc<100) display.setCursor(offset+displayWidth-50,InfoBarRow);
  ...
  if (RTC_bat_choice==0){
    if(displayHeight<130) {
                        display.setCursor(offset+displayWidth-60,InfoBarRow);
    } else {
                        display.setCursor(offset+displayWidth-75,InfoBarRow);
    }
    display.print(RTC_voltage_bat,2); display.print("V");
  }
}
void Bat_level_print(){
  float bat_perc=100*(1-(VOLTAGE_100-RTC_voltage_bat)/(VOLTAGE_100-VOLTAGE_0));
  if (bat_perc<0) bat_perc=0;
  if (bat_perc>100) bat_perc=100;
  if (RTC_bat_choice==1) { display.print(int(bat_perc)); display.print("%");}
  else               { display.print(RTC_voltage_bat,2);}
}
...
void Update_screen(int screen) {
  ...
  if (screen == GPS_INIT_SCREEN) {
    ...
    if(config.ublox_type==0xFF){
      SetFont_L();
      CURSOR(3,Font); display.print("Auto detect gps-type");
    }
...
//      DEVICE_BOOT_LOG(24);
      if (ubxMessage.navPvt.numSV < 5) {
        CURSOR(2,Font);              SetFont_M(); display.print("Waiting for Sat >=5"); SetFont_L();
...
//    DEVICE_BOOT_LOG(2);
 ...
 //   DEVICE_BOOT_LOG(2);
  ...
  if (screen == SPEED) {
    ...
    SetFont_M(); 
    CURSOR_COL(1,offset+displayWidth-2*Font,Font);
...
//    if(GPS_Signal_OK==false){    //////////// for test purposes - set bool test_screen = false in ESP_functions ////////////////
      ...
      if(config.speed_large_font==1)  SetFont_Speed_I();                            // smaller font for speed and bigger font for runs
      if(config.speed_large_font==2) {SetFont_Speed_I2(); display.setTextSize(2);}  // largest font for speed with 2x multiplication and smaller field for runs above decimals
      ...
      display.setCursor(offset+max(displayWidth/2-TextWidth/2,0),displayHeight-2); // center speed
      ...
      if(config.speed_large_font==0){
        ...
        if(config.speed_large_font==1){
          ...
          if(config.speed_large_font==2){
            SetFont_L();
            CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print("Run:"); SetFont_L(); display.print(S10.s_max_speed*calibration_speed,1);
            CURSOR_COL(2,displayWidth*0.62,Font); SetFont_M(); display.print("Avg:"); SetFont_L(); display.print(S10.avg_5runs*calibration_speed,1);
          }
        }
      }
    }
      ...
      if(config.speed_large_font==0){
        ...
        if(config.speed_large_font==1){
          ...
          if(config.speed_large_font==2){
            SetFont_L();
            if((alfa_window<99)&(Ublox.alfa_distance/1000<255)){                                //Window alleen indien Window<100 en Run>350 meter !!!!&(A500.alfa_speed_max*calibration_speed<1)
              CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print("Gat:"); display.print(alfa_window,0);
              CURSOR_COL(2,displayWidth*0.62,Font); SetFont_M(); display.print("Ex :"); display.print(alfa_exit,0);
            } else {
              CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print("Alf:"); SetFont_L(); 
              if(A500.alfa_speed_max*calibration_speed>1)                   display.print(S10.s_max_speed*calibration_speed,1);
              else                                                          display.print("MISS");
            }
          }
        }
      }
    ...
    if (field == 4) {                     //First line speed screen = nautical mile info
      if (config.speed_large_font == 0) {
        ...
        if(config.speed_large_font==1){
          ...
          if(config.speed_large_font==2){
            SetFont_L();
            CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print("NMa:"); SetFont_L(); display.print(M1852.m_max_speed*calibration_speed,1);
            CURSOR_COL(2,displayWidth*0.62,Font); SetFont_M(); display.print("NM :"); SetFont_L(); display.print(M1852.display_max_speed*calibration_speed,1);
          }
        }
      }
    }  
    if (field == 5) {
      if(config.speed_large_font==0){
        ...
        if(config.speed_large_font==1){
          ...
          SetFont_XXXL();                                     display.print(Ublox.total_distance/1000000,1);//Total distance in km, als test run_distance
          if(Ublox.total_distance/1000000>99.9){SetFont_M();  display.print("km");} 
          else                                 {SetFont_M();  display.print("m");}
          SetFont_M();    CURSOR2(2,Font); SetFont_XXL();     Bat_level_print();
          SetFont_XXL();
          SetBarPosition(Font);
        } else {
          if(config.speed_large_font==2){
            SetFont_L();
            CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print("Dis:"); SetFont_L(); display.print(Ublox.total_distance/1000000,1);
            CURSOR_COL(2,displayWidth*0.62,Font); SetFont_M(); display.print("Bat:"); SetFont_L(); Bat_level_print();
          }
        }
      }
    }
    if (field == 6) {
      if(config.speed_large_font==0){
        ...
        if(config.speed_large_font==1){
          ...
          if(config.speed_large_font==2){
            SetFont_L();
            CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print(" 2S:"); SetFont_L(); display.print(S2.display_max_speed*calibration_speed,1);
            CURSOR_COL(2,displayWidth*0.62,Font); SetFont_M(); display.print("10S:"); SetFont_L(); display.print(S10.display_max_speed*calibration_speed,1);
          }
        }
      }
    }
    if (field == 7) {
      if(config.speed_large_font==0){
        ...
        if(config.speed_large_font==1){
          ...
          if(config.speed_large_font==2){
            SetFont_L();
            CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print(".5hA:"); SetFont_L(); display.print(S1800.avg_s*calibration_speed,1);
            CURSOR_COL(2,displayWidth*0.62,Font); SetFont_M(); display.print(".5hB:"); SetFont_L(); display.print(S1800.display_max_speed*calibration_speed,1);
          }
        }
      }
    }
    if (field == 8) {
      if(config.speed_large_font==0){
        ...
        if(config.speed_large_font==1){
          ...
          if(config.speed_large_font==2){
            SetFont_L();
            CURSOR_COL(1,displayWidth*0.62,Font); SetFont_M(); display.print("1hA:"); SetFont_L(); display.print(S3600.avg_s*calibration_speed,1);
    ...
    if (field == 8) {
      ...
      if (log_seconds > 3600) { start_logging_millis = millis(); }     //60 minutes = full bar
    }
    if (bar_length) {
  ...
  if (screen == SPEED2) {
    ...
    if (GPS_Signal_OK == true) {
      ...
      if ((S2.s_max_speed*calibration_speed>S2.avg_speed[5]*calibration_speed)&(S2.s_max_speed*calibration_speed>config.start_logging_speed*1000)){
        display.fillScreen(GxEPD_BLACK);
        display.setTextColor(GxEPD_WHITE);
      } else {
        display.fillScreen(GxEPD_WHITE);
  ...
  if (screen == STATS3) {  //100m,250m, 500m,Alfa
    update_delay = (config.Stat_screens_time - 2) * 1000;
    SetFont_XL();
    CURSORC(2,Font);  display.print("100m"); CURSOR2C(2,Font); display.print(":");display.print(M100.avg_speed[9]*calibration_speed,2);      //best 2s, was avg_speed[9]
    CURSORC(3,Font);  display.print("250m"); CURSOR2C(3,Font); display.print(":");display.print(M250.display_max_speed*calibration_speed,2); //best 10s(Fast), was avg_speed[9]
    CURSORC(4,Font);  display.print("500m"); CURSOR2C(4,Font); display.print(":");display.print(M500.avg_speed[9]*calibration_speed,2);      //langzaamste 10s(Slow) run van de sessie
    CURSORC(5,Font);  display.print("Alfa"); CURSOR2C(5,Font); display.print(":");display.print(A500.avg_speed[9]*calibration_speed,2);      //average 5*10s
  }
  ...
  if (screen == STATS6) {  //Simon stat screen 10sec
    Serial.println("STATS6_Simon_screen 10sec");
    ...
    CURSOR_COLC(2,col4,Font); display.print(":");display.print(S10_previous_run*calibration_speed,2);
 ...
 // position line of actual 10s run
  ...
  if(screen==STATS8){ //Simon stat screen for 2nd speed target
    Serial.println("STATS8_Simon_screen");
    update_delay=(config.Stat_screens_time-2)*1500;
    int line;
    double S2avgNEW;
    double M500avgNEW;
    double M1852avgNEW;
    double A500avgNEW;
    if(config.Stat_speed_target2==1){
      if (S2.s_max_speed>S2.avg_speed[5]) S2avgNEW=(S2.avg_speed[9]+S2.avg_speed[8]+S2.avg_speed[7]+S2.avg_speed[6]+S2.s_max_speed)/5;
      else  S2avgNEW=(S2.avg_speed[9]+S2.avg_speed[8]+S2.avg_speed[7]+S2.avg_speed[6]+S2.avg_speed[5])/5;
    }
    if(config.Stat_speed_target2==2){
      double M500avgNEW;
        M500avgNEW=(M500.avg_speed[9]+M500.avg_speed[8]+M500.avg_speed[7]+M500.avg_speed[6]+M500.avg_speed[5])/5;
    }
    if(config.Stat_speed_target2==4){
      double M1852avgNEW;
      M1852avgNEW=(M1852.avg_speed[9]+M1852.avg_speed[8]+M1852.avg_speed[7]+M1852.avg_speed[6]+M1852.avg_speed[5])/5;
    }
    if(config.Stat_speed_target2==5){
      double A500avgNEW;
      A500avgNEW=(A500.avg_speed[9]+A500.avg_speed[8]+A500.avg_speed[7]+A500.avg_speed[6]+A500.avg_speed[5])/5;
    }
    SetCols(1);
    SetFont_LS();
    CURSOR_COLC(1,col1,Font); display.print("AV");
    ...
    if(config.Stat_speed_target2==1){
      CURSOR_COLC(1,col2,Font); display.print(":");display.print(S2avgNEW * calibration_speed, 2);
      CURSOR_COLC(2,col2,Font); display.print(":");display.print(S2.avg_speed[9]*calibration_speed,2);
      CURSOR_COLC(3,col2,Font); display.print(":");display.print(S2.avg_speed[8]*calibration_speed,2);
      CURSOR_COLC(4,col2,Font); display.print(":");display.print(S2.avg_speed[7]*calibration_speed,2); 
      CURSOR_COLC(5,col2,Font); display.print(":");display.print(S2.avg_speed[6]*calibration_speed,2);
      CURSOR_COLC(6,col2,Font); display.print(":");display.print(S2.avg_speed[5]*calibration_speed,2); 
      
      display.setTextSize(2);
      CURSOR_COLC(2,col3,Font); display.print("Prv 2");
      CURSOR_COLC(4,col3,Font); display.print(S2.display_last_run*calibration_speed,2);
      display.setTextSize(1);
     
      // position line of actual 2s run
      if      (S2.display_last_run*calibration_speed>S2.avg_speed[9]*calibration_speed) YPOS(1,Font);
      else if (S2.display_last_run*calibration_speed>S2.avg_speed[8]*calibration_speed) YPOS(2,Font);
      else if (S2.display_last_run*calibration_speed>S2.avg_speed[7]*calibration_speed) YPOS(3,Font);
      else if (S2.display_last_run*calibration_speed>S2.avg_speed[6]*calibration_speed) YPOS(4,Font);
      else if (S2.display_last_run*calibration_speed>S2.avg_speed[5]*calibration_speed) YPOS(5,Font);
      else YPOS(6,Font);
      display.fillRect(0,y_pos+2,col3-10,2,GxEPD_BLACK);//lijn voor actuele run weer te geven...
    }
    if(config.Stat_speed_target2==2){
      CURSOR_COLC(1,col2,Font); display.print(":");display.print(M500avgNEW * calibration_speed, 2);
      CURSOR_COLC(2,col2,Font); display.print(":");display.print(M500.avg_speed[9]*calibration_speed,2);
      CURSOR_COLC(3,col2,Font); display.print(":");display.print(M500.avg_speed[8]*calibration_speed,2);
      CURSOR_COLC(4,col2,Font); display.print(":");display.print(M500.avg_speed[7]*calibration_speed,2); 
      CURSOR_COLC(5,col2,Font); display.print(":");display.print(M500.avg_speed[6]*calibration_speed,2);
      CURSOR_COLC(6,col2,Font); display.print(":");display.print(M500.avg_speed[5]*calibration_speed,2); 

      display.setTextSize(2);
      CURSOR_COLC(2,col3,Font); display.print("500m");
//      CURSOR_COLC(4,col3,Font); display.print(":");display.print(M500.display_last_run*calibration_speed,2);
      display.setTextSize(1);
/*         
      // position line of actual 500m run
      if      (M500.s_max_speed*calibration_speed>M500.avg_speed[9]*calibration_speed) YPOS(1,Font);
      else if (M500.s_max_speed*calibration_speed>M500.avg_speed[8]*calibration_speed) YPOS(2,Font);
      else if (M500.s_max_speed*calibration_speed>M500.avg_speed[7]*calibration_speed) YPOS(3,Font);
      else if (M500.s_max_speed*calibration_speed>M500.avg_speed[6]*calibration_speed) YPOS(4,Font);
      else if (M500.s_max_speed*calibration_speed>M500.avg_speed[5]*calibration_speed) YPOS(5,Font);
      else YPOS(6,Font);
      display.fillRect(0,y_pos+2,col3-10,2,GxEPD_BLACK);//lijn voor actuele run weer te geven...
*/    }
    if(config.Stat_speed_target2==3){
      CURSOR_COLC(1,col2,Font); display.print(":");display.print(M1852avgNEW * calibration_speed, 2);
      CURSOR_COLC(2,col2,Font); display.print(":");display.print(M1852.avg_speed[9]*calibration_speed,2);
      CURSOR_COLC(3,col2,Font); display.print(":");display.print(M1852.avg_speed[8]*calibration_speed,2);
      CURSOR_COLC(4,col2,Font); display.print(":");display.print(M1852.avg_speed[7]*calibration_speed,2); 
      CURSOR_COLC(5,col2,Font); display.print(":");display.print(M1852.avg_speed[6]*calibration_speed,2);
      CURSOR_COLC(6,col2,Font); display.print(":");display.print(M1852.avg_speed[5]*calibration_speed,2); 
 
      display.setTextSize(2);
      CURSOR_COLC(2,col3,Font); display.print("1852m");
//      CURSOR_COLC(4,col3,Font); display.print(":");display.print(M1852.display_last_run*calibration_speed,2);
      display.setTextSize(1);
/*         
      // position line of actual 1852m run
      if      (M1852.s_max_speed*calibration_speed>M1852.avg_speed[9]*calibration_speed) YPOS(1,Font);
      else if (M1852.s_max_speed*calibration_speed>M1852.avg_speed[8]*calibration_speed) YPOS(2,Font);
      else if (M1852.s_max_speed*calibration_speed>M1852.avg_speed[7]*calibration_speed) YPOS(3,Font);
      else if (M1852.s_max_speed*calibration_speed>M1852.avg_speed[6]*calibration_speed) YPOS(4,Font);
      else if (M1852.s_max_speed*calibration_speed>M1852.avg_speed[5]*calibration_speed) YPOS(5,Font);
      else YPOS(6,Font);
      display.fillRect(0,y_pos+2,col3-10,2,GxEPD_BLACK);//lijn voor actuele run weer te geven...
*/    }
    if(config.Stat_speed_target2==4){
      CURSOR_COLC(1,col2,Font); display.print(":");display.print(A500avgNEW * calibration_speed, 2);
      CURSOR_COLC(2,col2,Font); display.print(":");display.print(A500.avg_speed[9]*calibration_speed,2);
      CURSOR_COLC(3,col2,Font); display.print(":");display.print(A500.avg_speed[8]*calibration_speed,2);
      CURSOR_COLC(4,col2,Font); display.print(":");display.print(A500.avg_speed[7]*calibration_speed,2); 
      CURSOR_COLC(5,col2,Font); display.print(":");display.print(A500.avg_speed[6]*calibration_speed,2);
      CURSOR_COLC(6,col2,Font); display.print(":");display.print(A500.avg_speed[5]*calibration_speed,2); 
 
      display.setTextSize(2);
      CURSOR_COLC(2,col3,Font); display.print("Alpha");
//      CURSOR_COLC(1,col4,Font); display.print(":");display.print(A500.display_last_run*calibration_speed,2);
      display.setTextSize(1);

/*         
      // position line of actual 1852m run
      if      (A500.s_max_speed*calibration_speed>A500.avg_speed[9]*calibration_speed) YPOS(1,Font);
      else if (A500.s_max_speed*calibration_speed>A500.avg_speed[8]*calibration_speed) YPOS(2,Font);
      else if (A500.s_max_speed*calibration_speed>A500.avg_speed[7]*calibration_speed) YPOS(3,Font);
      else if (A500.s_max_speed*calibration_speed>A500.avg_speed[6]*calibration_speed) YPOS(4,Font);
      else if (A500.s_max_speed*calibration_speed>A500.avg_speed[5]*calibration_speed) YPOS(5,Font);
      else YPOS(6,Font);
      display.fillRect(0,y_pos+2,col3-10,2,GxEPD_BLACK);//lijn voor actuele run weer te geven...
*/    }
  }
  ...
  if(screen==STATS9){ //Simon stat screen for 3rd speed target
    ...
    double M1852avgNEW;
    double A500avgNEW;
    if(config.Stat_speed_target3==1){
      if (S2.s_max_speed>S2.avg_speed[5]) S2avgNEW=(S2.avg_speed[9]+S2.avg_speed[8]+S2.avg_speed[7]+S2.avg_speed[6]+S2.s_max_speed)/5;
      else  S2avgNEW=(S2.avg_speed[9]+S2.avg_speed[8]+S2.avg_speed[7]+S2.avg_speed[6]+S2.avg_speed[5])/5;
    }
    if(config.Stat_speed_target3==2){
      double M500avgNEW;
        M500avgNEW=(M500.avg_speed[9]+M500.avg_speed[8]+M500.avg_speed[7]+M500.avg_speed[6]+M500.avg_speed[5])/5;
    }
    if(config.Stat_speed_target3==4){
      double M1852avgNEW;
      M1852avgNEW=(M1852.avg_speed[9]+M1852.avg_speed[8]+M1852.avg_speed[7]+M1852.avg_speed[6]+M1852.avg_speed[5])/5;
    }
    if(config.Stat_speed_target3==5){
      double A500avgNEW;
      A500avgNEW=(A500.avg_speed[9]+A500.avg_speed[8]+A500.avg_speed[7]+A500.avg_speed[6]+A500.avg_speed[5])/5;
    }
    SetCols(1);
    ...
    if(config.Stat_speed_target3==1){
      ...
      display.setTextSize(2);
      CURSOR_COLC(2,col3,Font); display.print("Prv 2sec");
      CURSOR_COLC(4,col3,Font); display.print(S2.display_last_run*calibration_speed,2);
      display.setTextSize(1);
...
/*         
      ...
      display.fillRect(0,y_pos+2,col3-10,2,GxEPD_BLACK);//lijn voor actuele run weer te geven...
*/    }
    if(config.Stat_speed_target3==3){
      CURSOR_COLC(1,col2,Font); display.print(":");display.print(M1852avgNEW * calibration_speed, 2);
      CURSOR_COLC(2,col2,Font); display.print(":");display.print(M1852.avg_speed[9]*calibration_speed,2);
      CURSOR_COLC(3,col2,Font); display.print(":");display.print(M1852.avg_speed[8]*calibration_speed,2);
      CURSOR_COLC(4,col2,Font); display.print(":");display.print(M1852.avg_speed[7]*calibration_speed,2); 
...
*/    }
  ...
  if(screen==STATS10){ //Motion BEST stat screen
