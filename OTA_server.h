...
void append_page_header() {
  ...
  webpage += F("<head>");
  webpage += F("<title>ESP-Logger</title>"); // NOTE: 1em = 16px
  ...
  webpage += F("</style></head><body><h1>ESP-Logger</h1>");
  webpage += F("<ul>");
  webpage += F("<a href='/firmware'>Firmware</a>");  
  webpage += F("</ul>");
}
  ...
  while(file){
    ...
    else
    {
      ...
      if(String(file.name()) != "config.txt"){
        ...
        webpage += F("' value='"); webpage +="download_"+String(file.name()); webpage +=F("'>Download</button>"); //webpage +=" <a href='/files/"+String(file.name())+"'>download</a>";
      }
      webpage += "</td>";
      webpage += "<td>";
      if(String(file.name()) != "config.txt"){
